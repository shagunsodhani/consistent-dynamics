...
from codes.model.imagination_model.recurrent_environment_simulator_obs_dependent_path_deterministic import Model as RESObsDependentPath

class Model(RESObsDependentPath):
    ...
    def forward(self, x):
        ...
        for t in range(0, trajectory_length):
            ...
            inp = torch.cat((action_fusion, obs_encoding), dim=1)
            current_output = self.state_transition_model(inp.unsqueeze(1))
            current_output = self.convolutional_decoder(current_output.squeeze(1))
            obs_encoding = self.convolutional_encoder(current_output)
            output.append(current_output.unsqueeze(1))
        reconstructed_obs = torch.cat(output, dim=1)
